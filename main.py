...
function parseContentResponse(data) {
    ...
    function addCategory(c) {
        ...
        let key = String(c.id !== undefined ? c.id : (c.name || JSON.stringify(c)));
        if (seenCats.has(key)) return;
        seenCats.add(key);
        categories.push(c);
    }
    ...
    function addProduct(p) {
        ...
        let key = String(p.id !== undefined ? p.id : JSON.stringify(p));
        if (seenProds.has(key)) return;
        seenProds.add(key);
        products.push(p);
    }
    ...
    function isProduct(item) {
        ...
        if (item.base_price !== undefined && item.base_price !== null) return true;
        if (item.product_type !== undefined) return true;
        return false;
    }
    ...
    function isCategory(item) {
        ...
        if (item.products || item.subcategories || item.sub_categories ||
            item.children || item.categories || item.sections) return true;
        if (item.name !== undefined && item.price === undefined) return true;
        if (item.id !== undefined && item.price === undefined && item.base_price === undefined) return true;
        return false;
    }
    
    function extractFromNode(node, depth) {
        if (depth > 20) return;
        if (!node) return;
            ...
            node.forEach(item => {
                if (item && typeof item === 'object') {
                    if (isProduct(item)) {
                        ...
                        addCategory(item);
                        extractChildrenFromCategory(item, depth + 1);
                    } else {
                        extractFromNode(item, depth + 1);
                    }
                }
            });
            return;
        ...
        if (typeof node === 'object') {
            ...
            if (isCategory(node)) {
                addCategory(node);
                extractChildrenFromCategory(node, depth + 1);
                return;
            }
            for (let k in node) {
                let v = node[k];
                if (Array.isArray(v) || (v && typeof v === 'object')) {
                    extractFromNode(v, depth + 1);
                }
            }
        }
    }
    
    function extractChildrenFromCategory(cat, depth) {
        if (!cat || depth > 20) return;
        
        let subKeys = ['subcategories', 'sub_categories', 'children', 'categories', 'sections'];
        subKeys.forEach(k => {
                ...
                cat[k].forEach(sc => {
                    if (sc && typeof sc === 'object') {
                        if (isProduct(sc)) {
                            ...
                            addCategory(sc);
                            extractChildrenFromCategory(sc, depth + 1);
                        }
                    }
                });
            }
            ...
            if (Array.isArray(cat[k])) {
                cat[k].forEach(p => {
                    if (p && typeof p === 'object') {
                        if (isProduct(p)) {
                            addProduct(p);
                        } else if (isCategory(p)) {
                            addCategory(p);
                            extractChildrenFromCategory(p, depth + 1);
                        }
                    }
                });
            }
...
function renderContentLevel(data) {
    ...
    categories.forEach(c => {
        ...
        if (Array.isArray(c.sub_categories)) cnt += c.sub_categories.length;
...
async function loadAllDescendants(rootId) {
    ...
    while (queue.length > 0 && iterations < MAX_ITERATIONS) {
        ...
        if (progressBar && progressFill) {
            ...
            let total = totalProcessed + queue.length + 1;
            progressFill.style.width = Math.min(95, (totalProcessed / total) * 100) + '%';
        }
        
        let infoEl = document.getElementById('importCountInfo');
        if (infoEl) {
            infoEl.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> جاري التحميل... (${totalProcessed} قسم تم فحصه، ${allProdsList.length} منتج، ${queue.length} في الانتظار)`;
        }
    }
    ...
    let seenIds = new Set();
    allProdsList = allProdsList.filter(p => {
        let k = String(p.id);
...
function buildTree(nodeId, nodeName, allCatsData, emojiId, color, desc, visited) {
    ...
    products.forEach(p => {
        let isCounterType = (p.product_type === "amount") &&
                            p.qty_values &&
                            typeof p.qty_values === 'object' &&
                            p.qty_values.min !== undefined;
...
function confirmImportCategory() {
    ...
    let totalSubs = 0;
    let maxDepth = 0;
    function countAll(sec, depth) {
        if (depth > maxDepth) maxDepth = depth;
        totalProducts += (sec.products || []).length;
        totalSubs += (sec.sub_categories || []).length;
        (sec.sub_categories || []).forEach(sub => countAll(sub, depth + 1));
    }
    countAll(rootSection, 0);
    
    let linkedCount = botData.api_products.length;
    alert(
        `✅ تم استيراد "${selectedImport.name}" بنجاح:\n` +
        `• الأقسام الرئيسية: 1\n` +
        `• الأقسام الفرعية: ${totalSubs}\n` +
        `• إجمالي المنتجات: ${totalProducts}\n` +
        `• أقصى عمق: ${maxDepth} مستوى\n` +
        `• الفئات المربوطة بـ API: ${linkedCount}\n` +
        `• الأقسام التي تم فحصها: ${Object.keys(allDescendantsCache.allCatsData).length}`
    );
    cancelImport();
}
